...
+ (BOOL)hideHUDForViewForHome:(UIView *)view animated:(BOOL)animated {
...
- (void)hideFohome:(BOOL)animated {
    ...
    if (self.minShowTime > 0.0 && showStarted) {
        ...
        if (interv < self.minShowTime) {
            self.minShowTimer = [NSTimer scheduledTimerWithTimeInterval:(self.minShowTime - interv)
                                                                 target:self
                                                               selector:@selector(handleMinShowTimerForhome:)
                                                               userInfo:nil
                                                                repeats:NO];
            return;
        }
    ...
    // ... otherwise hide the HUD immediately
    [self hideUsingAnimationForHome:useAnimation];
}
- (void)hideUsingAnimationForHome:(BOOL)animated {
    // Fade out
    if (animated) {
